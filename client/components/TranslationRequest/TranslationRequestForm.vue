      ...
      <v-form v-model="isFormValid" @submit.prevent="submitRequest()">
        <v-container class="form-container">
            ...
            <v-row>
              <v-col class="col">
                <div v-for="(author, idx) in authors" v-bind:key="`author-${idx}`" class="row">
                  <v-text-field :rules="nonEmptyRule" :label="`Author #${idx + 1} (First)`" v-model="author.first"
                    color="#95AEB3"></v-text-field>
                  <v-text-field :rules="nonEmptyRule" :label="`Author #${idx + 1} (Last)`" v-model="author.last"
                    color="#95AEB3"></v-text-field>
          ...
          <div class="form-section">
            ...
            <v-row>
              <v-col> <v-textarea label="Request description" v-model="description"
                  :placeholder="`Provide more context or describe what you need help with.`" color="#95AEB3"></v-textarea>
              </v-col>
            </v-row>
          </div>
